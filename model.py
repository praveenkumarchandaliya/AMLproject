...
class DCGAN(object):
    ...
    def train(self, config):
        ...
        sample_z[0:self.sample_size//2][0:self.z_dim//2] = sample_z[0:self.sample_size//2][0:self.z_dim//2]/1.5
        sample_z[self.sample_size//2:self.sample_size][self.z_dim//2:self.z_dim] = sample_z[self.sample_size//2:self.sample_size][self.z_dim//2:self.z_dim]/1.5
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_z = np.random.uniform(-1, 1, [config.batch_size, self.z_dim]) \
                            .astype(np.float32)
                batch_z[0:config.batch_size//2][0:self.z_dim//2] = batch_z[0:config.batch_size//2][0:self.z_dim//2]/1.5
                batch_z[config.batch_size//2:config.batch_size][self.z_dim//2:self.z_dim] = batch_z[config.batch_size//2:config.batch_size][self.z_dim//2:self.z_dim]/1.5
